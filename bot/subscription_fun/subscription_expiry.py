...
@log_function_call(logger)
async def notify_user_about_expiring_subscription(user_id, telegram_id, end_date):
    """
    Отправляет уведомление пользователю о том, что его подписка истекает завтра.
    После отправки добавляет запись уведомления в базу.
    """
...
async def notify_expiring_subscriptions():
    ...
    for sub in subs:
        ...
        await notify_user_about_expiring_subscription(user_id, telegram_id, end_date)
